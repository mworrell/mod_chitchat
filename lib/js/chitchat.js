...
function Chitchat ()
{
    ...
    setTimeout(function() {
        pubzub.subscribe("~site/chitchat/status", function(topic, msg) { self.receive_status(topic, msg, false); });
        pubzub.subscribe("~site/chitchat/msg/+", function(topic, msg) { self.receive_msg(topic, msg); });
        pubzub.subscribe("~site/chitchat/direct/"+self._client_id, function(topic, msg) { self.receive_direct(topic, msg); });
        pubzub.lastwill("~site/chitchat/status", lastwill);
        window.onbeforeunload = function() {
            pubzub.publish("~site/chitchat/status", lastwill);
        };
...
Chitchat.prototype.send_direct = function(client_id, msg) {
    pubzub.publish('~site/chitchat/direct/'+client_id, msg);
};
...
Chitchat.prototype.set_status = function(room, status) {
    ...
    pubzub.publish("~site/chitchat/status", this._status);
    if (changed) {
        this.fetch_msg(room);
    }
};
...
Chitchat.prototype.message = function(msg) {
    pubzub.publish("~site/chitchat/msg/"+this._status.room, {
